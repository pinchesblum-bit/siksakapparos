...
create or replace function public.kapparos_allocate_ticket_id(p_sales jsonb, p_candidate text default null)
...
$$;
revoke all on function public.kapparos_allocate_ticket_id(jsonb,text) from public,anon,authenticated;
grant execute on function public.kapparos_allocate_ticket_id(jsonb,text) to service_role;
...
begin
  ...
  -- Allocate short IDs under the same row lock used by online checkout.
  for item in select value from jsonb_array_elements(merged_sales) loop
    previous_sale := null;
    select value into previous_sale from jsonb_array_elements(current_sales)
    ...
    item := jsonb_set(item,'{ticketId}',to_jsonb(ticket));
    assigned_sales := assigned_sales || jsonb_build_array(item);
  end loop;
  merged_sales := assigned_sales;
