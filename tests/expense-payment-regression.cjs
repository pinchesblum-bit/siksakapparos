...
(async()=>{
 ...
 open(expense);confirm=false;before=confirmations;fields.expensePaid.checked=false;await env.expensePaidHandler();assert.equal(confirmations,before+1);assert.equal(expense.paid,true);assert.equal(fields.expensePaid.checked,true);assert.equal(expense.paymentType,'check','cancel unpaid retains payment');assert.equal(expense.paymentDetails,'Check #1042');assert.equal(form.classList.contains('expense-readonly'),true);
 ...
 // After marking unpaid, Paid opens with blank payment information.
 before=confirmations;fields.expensePaid.checked=true;await env.expensePaidHandler();assert.equal(confirmations,before);assert.equal(form.classList.contains('expense-readonly'),false);assert.equal(expense.paid,false);assert.equal(fields.expensePaymentType.value,'');assert.equal(fields.expensePaymentDetails.value,'');assert.equal(expense.paymentType,'');assert.equal(expense.paymentDetails,'');env.closeExpenseModal();assert.equal(expense.paid,false,'cancel does not mark paid');
 // Chicken expense keeps its protected name but allows a manual total amount.
 expense={...base(),id:'auto-chicken-inventory-cost',name:'Chickens',amount:1000};open(expense);before=confirmations;fields.expensePaid.checked=true;await env.expensePaidHandler();assert.equal(confirmations,before);assert.equal(fields.expenseName.disabled,true);assert.equal(fields.expenseAmount.disabled,false);assert.equal(fields.expensePaymentType.disabled,false);fields.expenseAmount.value='975.25';fields.expensePaymentType.value='cash';await submit();assert.equal(expense.paid,true);assert.equal(expense.amount,975.25);assert.equal(env.state.settings.chickenExpenseAmountOverride,975.25);
 ...
 expense={...base(),paid:true,paymentType:'credit',paymentDetails:'Visa ending 9999'};open(expense);env.showExpenseEditor(expense);fields.expensePaid.checked=false;await env.expensePaidHandler();assert.equal(fields.expensePaymentType.value,'');assert.equal(fields.expensePaymentDetails.value,'');fields.expenseNote.value='Edited unpaid';await submit();assert.equal(expense.note,'Edited unpaid');assert.equal(expense.paid,false);assert.equal(expense.paymentType,'');assert.equal(expense.paymentDetails,'');
 console.log('PASS: Paid opens the editor without confirmation, including stored methods; payment validation and Save confirmation; Unpaid confirms immediately; cancel keeps saved state; chicken total is editable while its name stays protected.');
